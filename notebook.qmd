...
idx_phe = colnames(mat) %in% as.character(phe_behav$index)
```
...
```{r}
p.fac_phe = bind_cols(phe[idx_phe,], as.data.frame(fac_phe)) %>%
  ggplot() +
...
```{r}
svd_phe_scores = bind_cols((phe[idx_phe,][rep(seq_len(nrow(phe[idx_phe,])), times = ncol(fac_phe)), , drop = FALSE]),
...
svd_var_scores = svd_var_scores %>%
  merge((var_ann %>% dplyr::select(snp,gene_id,gene_symbol)), 
        by = "snp", 
        all.x = T) %>%
...
svd_gwa_rem_dtf = svd_gwa_rem %>%
  ...
  mutate(color = if_else(chr %% 2 == 1,
                         pal[2],
                         pal[3])) %>%
  merge((var_ann %>% dplyr::select(snp,gene_symbol)), by = "snp", all.x = T) %>%
