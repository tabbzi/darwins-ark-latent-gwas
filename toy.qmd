...
The following toy example of this data set includes only GWAS summary statistics for association studies of survey items contributing to factor 6 (biddability), factor 24 (separation anxiety), and two physical traits (height and
...
Genomic associations for morphology traits are very strong, and may bias?
...
```{r}
pca_res_var = FactoMineR::PCA(t(mat), 
                          scale.unit = TRUE, 
                          ncp = 10, 
                          graph = FALSE)

pca_res_ind = FactoMineR::PCA(mat, 
                          scale.unit = TRUE, 
                          ncp = 10, 
                          graph = FALSE)
...
```{r}
fviz_pca_ind(pca_res, col.var="contrib") +
  theme_minimal()
```

```{r}
fviz_pca_biplot(pca_res, geom = "text")
```

## Exploratory factor analysis

### Perform EFA with FactoMineR
